...
const AksamMenuScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const querySnapshot = await getDocs(query(collection(db, 'aksamMenu'), orderBy('date', 'asc')));
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(query(collection(db, 'aksamMenu'), orderBy('date', 'asc')), (snapshot) => {
  ...
  const sendData = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'aksamMenu'), {
        date: nextDate,
        foods: [newFood],
      });
  ...
  const deleteData = async (id) => {
    setLoading(true);
    try {
      await deleteDoc(doc(db, 'aksamMenu', id));
  ...
  const updateData = async () => {
    if (selectedDocId) {
      setLoading(true);
      try {
        const itemDoc = doc(db, 'aksamMenu', selectedDocId);
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <View style={styles.container}>
          <Text style={styles.title}>AKŞAM</Text>
          {loading ? (
...
export default AksamMenuScreen;
