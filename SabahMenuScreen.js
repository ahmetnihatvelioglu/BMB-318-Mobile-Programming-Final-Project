...
const SabahMenuScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const querySnapshot = await getDocs(query(collection(db, 'sabahMenu'), orderBy('date', 'asc')));
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(query(collection(db, 'sabahMenu'), orderBy('date', 'asc')), (snapshot) => {
  ...
  const sendData = async () => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(db, 'sabahMenu'), {
        date: nextDate, 
        foods: [newFood],
      });
  ...
  const deleteData = async (id) => {
    setLoading(true);
    try {
      await deleteDoc(doc(db, 'sabahMenu', id));
  ...
  const updateData = async () => {
    if (selectedDocId) {
      setLoading(true);
      try {
        const itemDoc = doc(db, 'sabahMenu', selectedDocId);
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <View style={styles.container}>
          <Text style={styles.title}>SABAH</Text>
          {loading ? (
...
export default SabahMenuScreen;
